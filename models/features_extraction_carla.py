        ...
        if scenario_type in scenario_key:
            ...
            for scenario_id in os.listdir(curr_path):
                ...
                for variant_name in os.listdir(curr_path):
                    ...
                    # Align frame number
                    index = -1
                    while img_file_list[-1][-7:-4]!=bbox_file_list[index][-8:-5]:
                        index -= 1
                    if index!=-1:
                        bbox_file_list = bbox_file_list[:,index+1]
                    # If video length < 100
                    if len(bbox_file_list)<100:
                        continue
...
def run_model(models, inputs, batch_n):
    ...
    return roi, Boxes.cat(roi_input).tensor.view(batch_n,20,-1)
    # batch_data_flat[:,frame_i*img_per_batch:(frame_i+1)*img_per_batch] = roi.cpu().numpy()
    # features_maskformer = features_maskformer.unsqueeze(0).view(1,batch_n,-1)
