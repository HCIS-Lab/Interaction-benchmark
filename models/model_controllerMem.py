...
class model_controllerMem(nn.Module):
    ...
    def init_memory(self, data_train):
        ...
        for i in range(self.num_prediction + 1):
            ...
            #print(data_train[j][2].shape)
            past = data_train[j][0].unsqueeze(0)
            future = data_train[j][1].unsqueeze(0)
            #future = data_train[j][2]
            #print(future.shape)
            #print("past:", past, "future:", future)
            #print(data_train[j][0], "and", data_train[j][1], "and", data_train[j][2])
            past = past.cuda()
            future = future.cuda()
    ...
    def forward(self, past, future=None):
        ...
        #print("output_past:", output_past, "state_past:", state_past)
        ...
        if future is not None:
            ...
            print("pred:", prediction.shape,"fut:", future_rep.shape)
