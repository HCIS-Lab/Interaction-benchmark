...
class CNNLSTM_maskformer(nn.Module):
    def __init__(self, num_cam, num_ego_class, num_actor_class, road):
        ...
        self.conv1 = nn.Sequential(
                ...
                nn.Conv2d(2048, 1536, kernel_size=3, stride=1, padding='same'),
                nn.BatchNorm2d(1536),
                nn.ReLU(inplace=True),
                nn.Conv2d(1536, 1024, kernel_size=3, stride=1, padding='same'),
                nn.BatchNorm2d(1024),
                nn.ReLU(inplace=True),
                nn.Conv2d(1024, 512, kernel_size=3, stride=1, padding='same'),
                nn.BatchNorm2d(512),
                nn.ReLU(inplace=True),
                nn.Conv2d(512*self.num_cam, 256, kernel_size=1, stride=1, padding='same'),
                nn.BatchNorm2d(256),
                nn.ReLU(inplace=True),
                    )
    ...
    def train_forward(self, x, tops=False, front_only=True):
        ...
        for i in range(seq_len):
            ...
            x_feature_i = x_feature_i.view(batch_size*self.num_cam, 1, 256)
