...
from Interaction_benchmark.datasets.retrieval import Retrieval_Data
...
class Engine(object):
	...
	def train(self):
		...
		for data in tqdm(dataloader_train):
			...
			pred_ego, pred_actor = model(fronts+lefts+rights)
			criterion = nn.CrossEntropyLoss()
			loss = criterion(pred_ego, ego) + criterion(pred_actor, actor)
			loss.backward()
...
# Model
model = CNNLSTM(config, args.device)
