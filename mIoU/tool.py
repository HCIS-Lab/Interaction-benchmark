...
def get_batch_iou(preds, binimgs,temp):
...
def get_val_info(model, valloader, loss_fn, device, use_tqdm=False):
    ...
    #loader2 = tqdm(valloader2) if use_tqdm else valloader2

    #controller = RawController(4).to(device)

    #to_heatmap = ToHeatmap(int(5))
    with torch.no_grad():
        for batch in loader:

            allimgs, rots, trans, intrins, post_rots, post_trans, binimgs = batch
            #img, topdown, points, target, actions, meta = batch[1]
            ...
            # first model
            pred_segs = model(allimgs.to(device), rots.to(device), trans.to(device), intrins.to(device),
            ...
            # print(pred_segs)
            # second mocel
            mi, temp = torch.max(segs, dim=1)
            mi = mi.view((segs.shape[0], 1, 200, 200))
            drivable_area = torch.zeros_like(mi)
            drivable_area[mi > 0.5] = 1
            non_drive = torch.ones((segs.shape[0], 1, 200, 200)).cuda() - drivable_area
            new_class = torch.ones((segs.shape[0], 1, 200, 200)).cuda()
            new_class[:, :, :100, :] = 0
            ...
            # print("pred_ca.shape= ", pred_cat.shape)
            # print("topdown.shape= ",topdown.shape)
            
            # pred_cat = pred_cat[:100,:,:,:]

            # target_heatmap = to_heatmap(target, pred_cat)[:, None].long()

            # out = model2(torch.cat((topdown, target_heatmap), 1))

            # alpha = torch.rand(out.shape).type_as(out)
            # between = alpha * out + (1-alpha) * points
            # out_cmd = controller(between)
            ...
            # loss2 = (loss_point + 0.1 * loss_cmd).mean()


            # total loss
            # total_seg_loss += loss_fn(pred_segs[:,:,:100,:], binimgs[:,:,:100,:]).item() * pred_segs.shape[0]
            # total_loss = total_loss+ loss_fn(pred_segs[:,:,:100,:], binimgs[:,:,:100,:]).item() * pred_segs.shape[0] + loss2
            total_lss_loss += loss_fn(pred_segs[:,:,:100,:], binimgs[:,:,:100,:]).item() * pred_segs.shape[0]
            # total_lbc_loss += loss2 * pred_segs.shape[0]
            #total_control_loss += loss_fn(preds_controls, controls) * preds_controls.shape[0]

            # iou
            intersect, union, _ = get_batch_iou(segs[:,:,:100,:], binimgs[:,:,:100,:],temp[:,:100,:])
            total_intersect += intersect
            total_union += union
    total_loss = total_lss_loss + total_lbc_loss
    model.train()
    return {
            'total_loss': total_loss / (len(valloader.dataset)),
            'lss_loss': total_lss_loss / (len(valloader.dataset)),
            'lbc_loss': total_lbc_loss / (len(valloader.dataset)),
