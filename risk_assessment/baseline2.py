    ...
    with torch.no_grad():
        for data in loader:
            ...
            for i,label in enumerate(labels):
                ...
                print(file_name)
                if s_type != 2:
                    GT_frame, gt_id  = GT_loader.getGTframe(temp[0],temp[1],temp[3])
                else:
                    GT_frame = None
                # bbox files
                bbox_list = sorted(os.listdir(os.path.join(nas_path,file_name, 'bbox/front')))
                ...
                for k,th in enumerate(thresh):
                    ...
                    for accumulate in range(1,5):
                        ...
                        for j in range(accumulate-1,curr_length):
                            flag = True
                            frame_num = str(int(bbox_list[j].split('.')[0]))
                            ...
                            for jj,data in enumerate(datas):
                                ...
                                if flag:
                                    ...
                                    temp[data['actor_id']] = weight[j][jj]
                                else:
                                    temp[data['actor_id']] = 0.0
...
def demo_specific(s_type,s_id,variant,model_path, local_path,nas_path,ablation):
    clip_time = 60
    write_video = False
    cmap = plt.get_cmap('tab20b')
    colors = [cmap(i)[:3] for i in np.linspace(0, 1, 20)]
    ...
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    net = baseline2_model.Baseline_SA(20,device,ablation,n_frame=clip_time,features_size=256*7*7) 
    net= torch.nn.DataParallel(net).to(device)
    net.load_state_dict(torch.load(model_path))
    net.eval()
    features = carla_dataset.get_specific_data(2,clip_time,local_path,nas_path,None,s_type,s_id,variant)   
    with torch.no_grad():
        preds, probs, _= net(features[1].unsqueeze(0),features[0].unsqueeze(0))
        out = None
        pred = preds[0].cpu().numpy()
        weight = probs[0].cpu().numpy()
        file_name = os.path.join(s_type,s_id,"variant_scenario",variant)
        plt.plot(pred[:60,1],linewidth=3.0)
        plt.ylim(0, 1)
        plt.ylabel('Probability')
        plt.xlabel('Frame')
        ...
        if out is not None:
            out.release()
        if True:   
            frame = img_archive.read(img_file_list[33])
            frame = cv2.imdecode(np.frombuffer(frame, np.uint8), cv2.IMREAD_COLOR)
            frame = cv2.resize(frame, (640, 360))
            json_file = open(os.path.join(nas_path,file_name, 'bbox/front', bbox_list[33]))
            datas = json.load(json_file)
            now_weight = new_weight[33,:]
            actor = []
            risk_score = []
            bar_color = []
            for num_box,data in enumerate(datas):
                color = colors[num_box % len(colors)]
                color_2 = [i  for i in color]
                color = [i * 255 for i in color]
                box = data['box']
                box = np.round(np.array(box)*0.5)
                bar_color.append(tuple(color_2[::-1]))
                actor.append(data['actor_id'])
                risk_score.append(round(now_weight[num_box]/255.0*10000)/10000)
                cv2.rectangle(frame,(int(box[0]),int(box[1])),(int(box[2]),int(box[3])),color,2)
                cv2.putText(frame,str(data['actor_id']),(int(box[2]),int(box[3])), cv2.FONT_HERSHEY_SIMPLEX, 0.5,(0,0,200),1,cv2.LINE_AA)
        cv2.imwrite('{}_{}_{}_demo.png'.format(s_type,s_id,variant),frame)
        # df = pd.DataFrame(actor, columns=['Object_id', 'Risk_score'])
        # ax = sns.barplot(x='Object_id', y='Risk_score', data=df,palette=bar_color)
        # ax.set_title('Risks')
        x_pos = np.arange(len(actor))
        ...
        plt.bar(x_pos, risk_score, color=bar_color)
        plt.xticks(x_pos, actor)
        plt.savefig('{}_{}_{}_risk_score.png'.format(s_type,s_id,variant))

def metric(batch_size, model_path, local,nas,ablation,clip_time,tracking=None):
    single = False
    if tracking is not None:
        object_perframe = tracking
    else:
    ...
    net.load_state_dict(torch.load(model_path))
    net.eval()   
    _,testloader,validationloader = carla_dataset.get_dataset_loader(2,local,nas,tracking,batch_size,detection_collate,clip_time,validation=True)
    test_total_batch = len(testloader)
    val_total_batch = len(validationloader)
    all_pred = None
    all_labels = None
    with torch.no_grad():
        for i,data in enumerate(validationloader):
            print("\tBatch : %d/%d" % (i+1,val_total_batch),end='\r')
            frame,roi,labels,_ = data
            frame = frame.to(device)
            roi = roi.to(device)
            labels = labels.to(device)
            pred, _, _= net(roi,frame)
            if i==0 :
                all_pred = pred[:,:int(clip_time*0.8),1].cpu().numpy()
                all_labels = labels.cpu().numpy()
            else:
                all_pred = np.concatenate((all_pred,pred[:,:int(clip_time*0.8),1].cpu().numpy()), axis=0)
                all_labels = np.concatenate((all_labels,labels.cpu().numpy()), axis=0)
        
        evaluation(all_pred,all_labels,total_time=int(clip_time*0.8))

def evaluation(all_pred,all_labels, total_time = 48, vis = True, length = None):
...
if __name__ == '__main__':
    ...
    elif args.mode == "demo_specific":
        s_type = input("Scenario type: ")
        s_id = input("Scenario id: ")
        variant = input("Variant: ")
        demo_specific(s_type,s_id,variant,model_path, local,nas,ablation)
    elif args.mode == "testing":
        testing(batch_size, model_path, local,nas,ablation)
    elif args.mode == "evaluation":
        metric(batch_size, model_path, local,nas,ablation,clip_time)
    elif args.mode == "guan":
        th = 0.8
        accumulate_frame = 2
        guan_lun(batch_size, model_path, local,nas,ablation,th,accumulate_frame,clip_time=clip_time)
